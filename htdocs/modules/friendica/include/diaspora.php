...
require_once($GLOBALS['xoops']->path("/modules/friendica/include/bb2diaspora.php"));
require_once($GLOBALS['xoops']->path("/modules/friendica/include/contact_selectors.php"));
require_once($GLOBALS['xoops']->path("/modules/friendica/include/queue_fn.php"));


function diaspora_dispatch_public($msg) {

	$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.* FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid` IN ( SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`network` = %s AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`addr` = %s ) AND `account_expired` = 0 ",
...
function diaspora_get_contact_by_handle($uid,$handle) {
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `network` = %s AND `uid` = %d AND `addr` = %s LIMIT 1",
		dbesc(NETWORK_DIASPORA),
		intval($uid),
		dbesc($handle)
	);
...
function diaspora_msg_build($msg,$user,$contact,$prvkey,$pubkey,$public = false) {
	...
	if(! $pubkey) {
		logger("diaspora_msg_build: pubkey missing: `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` id: " . $contact['id']);
		return '';
	}
 ...
 *   array $importer -> FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` table
...
function diaspora_request($importer,$xml) {
	...
	if($contact) {
		...
		if($contact['rel'] == CONTACT_IS_FOLLOWER  &&  $importer['page-flags'] != PAGE_COMMUNITY) {
			q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `rel` = %d, `writable` = 1 WHERE `id` = %d AND `uid` = %d LIMIT 1",
	...
	$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact")  . "` ( `uid`, `network`, `addr`, `created`, `url`, `nurl`, `batch`, `name`, `nick`, `photo`, `pubkey`, `notify`, `poll`, `blocked`, `priority` )
	...
	if(! $contact_record) {
		logger("diaspora_request: unable to locate newly created `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` record.");
		return;
	}
	...
	if($importer['page-flags'] == PAGE_NORMAL) {
		...
		$u = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE uid = %d LIMIT 1",intval($importer['uid']));
		if($u)
...
function diaspora_post($importer,$xml) {
	...
	$r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `uid` = %d AND `uri` = %s AND `guid` = %s LIMIT 1",
		intval($importer['uid']),
		dbesc($message_id),
		dbesc($guid)
	);
	...
	$g = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "guid") . "` WHERE guid = %s LIMIT 1",
		dbesc($guid)
	);
	if(! count($g)) {
		q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "guid") . "  ( guid ) values ( %s )",
			dbesc($guid)
		);
	}
	...
	if($message_id) {
		q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET plink = %s WHERE id = %d LIMIT 1",
...
function diaspora_comment($importer,$xml,$msg) {
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `uid` = %d AND `guid` = %s LIMIT 1",
		intval($importer['uid']),
		dbesc($guid)
	);
	if(count($r)) {
...
function diaspora_conversation($importer,$xml,$msg) {
	...
	if(count($c))
		...
		$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "conv") . "` (uid,guid,creator,created,updated,subject,recips) values(%d, %s, %s, %s, %s, %s, %s) ",
			intval($importer['uid']),
			dbesc($guid),
			dbesc($diaspora_handle),
	...
	foreach($messages as $mesg) {
		...
		q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mail") . "` ( `uid`, `guid`, `convid`, `from-name`, `from-photo`, `from-url`, `contact-id`, `title`, `body`, `seen`, `reply`, `uri`, `parent-uri`, `created` ) values ( %d, %s, %d, %s, %s, %s, %d, %s, %s, %d, %d, %s, %s, %s)",
			intval($importer['uid']),
			dbesc($msg_guid),
		...
		q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "conv") . "` SET updated = %s WHERE id = %d LIMIT 1",
			dbesc(datetime_convert()),
			intval($conversation['id'])
		);		
...
function diaspora_photo($importer,$xml,$msg) {
	...
	$contact = diaspora_get_contact_by_handle($importer['uid'],$msg['author']);
	if(! $contact) {
		logger("diaspora_photo: `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` record not found: " . $msg['author'] . ' handle: ' . $diaspora_handle);
		return;
	}
	...
	if(strpos($parent_item['body'],$link_text) === false) {
		$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `body` = %s, `visible` = 1 WHERE `id` = %d AND `uid` = %d LIMIT 1",
...
function diaspora_like($importer,$xml,$msg) {
	...
	if(count($r)) {
		...
		if($positive === 'false') {
			q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `deleted` = 1 WHERE `id` = %d AND `uid` = %d LIMIT 1",
				intval($r[0]['id']),
				intval($importer['uid'])
			);
			// FIXME
...
function diaspora_retraction($importer,$xml) {
	...
	elseif($type === 'Post') {
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE guid = %s AND uid = %d LIMIT 1",
			dbesc('guid'),
			intval($importer['uid'])
		);
		if(count($r)) {
...
function diaspora_profile($importer,$xml) {
	...
	$r = q("SELECT DISTINCT ( `resource-id` ) FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE  `uid` = %d AND `contact-id` = %d AND `album` = 'Contact Photos' ",
		intval($importer['uid']),
		intval($contact['id'])
	);
	...
	// Not doing this currently because D* protocol is scheduled for revision soon. 
		...
		$r = q("UPDATE `". $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `name` = %s, `name-date` = %s, `photo` = %s, `thumb` = %s, `micro` = %s, `avatar-date` = %s , `bd` = %s WHERE `id` = %d AND `uid` = %d LIMIT 1",
		dbesc($name),
		dbesc(datetime_convert()),
		dbesc($images[0]),
		dbesc($images[1]),
	...
	if($r) {
		if($oldphotos) {
			foreach($oldphotos as $ph) {
				q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `contact-id` = %d AND `album` = 'Contact Photos' AND `resource-id` = %s ",
					intval($importer['uid']),
					intval($contact['id']),
...
function diaspora_send_followup($item,$owner,$contact,$public_batch = false) {
	...
	$p = q("SELECT guid FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE parent = %d LIMIT 1",
		$item['parent']
	);
	if(count($p))
...
function diaspora_send_relay($item,$owner,$contact,$public_batch = false) {
	...
	if(count($r)) { 
		...
		$itemcontact = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `id` = %d LIMIT 1",
