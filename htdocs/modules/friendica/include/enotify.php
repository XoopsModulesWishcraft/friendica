...
//		logger("notification: params = " . print_r($params, true), LOGGER_DEBUG);
		...
		if($params['otype'] === 'item'  &&  $parent_id) {
			$p = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE id = %d AND uid = %d LIMIT 1",
				intval($parent_id),
				intval($params['uid'])
			);
		}
	...
	if($params['type'] == NOTIFY_intro) {
		...
		$preamble = sprintf( t('You\'ve received anintroduction FROM \'%s\' at %s'), $params['source_name'], $sitename); 
		$epreamble = sprintf( t('You\'ve received %s FROM %s.'), '[url=$itemlink]' . t('anintroduction') . '[/url]' , '[url=' . $params['source_link'] . ']' . $params['source_name'] . '[/url]'); 
		$body = sprintf( t('You may visit their profile at %s'),$params['source_link']);

		$sitelink = t('Please visit %s to approve or reject theintroduction.');
		$tsitelink = sprintf( $sitelink, $siteurl );
		$hsitelink = sprintf( $sitelink, '<a href="' . $siteurl . '">' . $sitename . '</a>');
		$itemlink =  $params['link'];
	}
	...
	do {
		$dups = false;
		$hash = random_string();
        $r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "notify") . "` WHERE `hash` = %s LIMIT 1",
			dbesc($hash));
		if(count($r))
			$dups = true;
	} while($dups == true);
	...
	$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "notify") . "` (hash,name,url,photo,date,uid,link,parent,type,verb,otype)
	...
	$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "notify") . "` SET msg = %s WHERE id = %d AND uid = %d LIMIT 1",
		dbesc($msg),
		intval($notify_id),
		intval($params['uid'])
	);
	...
	if(intval($params['notify_flags']) & intval($params['type'])) {
		...
		$r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "notify_threads") . "` WHERE `master-parent-item` = %d AND `receiver-uid` = %d LIMIT 1",	intval($params['parent']),	intval($params['uid']) );
...
intval($params['uid']), LOGGER_DEBUG);
			$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "notify_threads") . "` ( `notify-id`, `master-parent-item`, `receiver-uid`, `parent-item` )
				values(%d, %d, %d, %d)",
				intval($notify_id),
