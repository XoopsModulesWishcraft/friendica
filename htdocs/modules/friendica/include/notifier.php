...
function notifier_run($argv, $argc){
	...
	if($cmd === 'mail') {
		$normal_mode = false;
		$mail = true;
		$message = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mail") . "` WHERE `id` = %d LIMIT 1",
				intval($item_id)
		);
		if(! count($message)){
			return;
	...
	elseif($cmd === 'suggest') {
		...
		$suggest = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fsuggest") . "` WHERE `id` = %d LIMIT 1",
			intval($item_id)
		);
		if(! count($suggest))
			return;
		...
		$items = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "sign") . "`.`signed_text`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "sign") . "`.`signature`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "sign") . "`.`signer`
			FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "sign") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "sign") . "`.`iid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`id` WHERE `parent` = %d AND visible = 1 AND moderated = 0 ORDER BY `id` ASC",
			intval($parent_id)
		);
	...
	if(! ($mail || $fsuggest)) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `id` IN ( $conversant_str ) AND `blocked` = 0 AND `pending` = 0");
	...
	elseif($fsuggest) {
		...
		q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fsuggest") . "` WHERE `id` = %d LIMIT 1",
			intval($item['id'])
		);
	...
	if(! $mail_disabled) {
		if((! strlen($target_item['allow_cid']))  &&  (! strlen($target_item['allow_gid'])) 
			...
			$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `uid` = %d AND `network` = %s",
				intval($uid),
				dbesc(NETWORK_MAIL)
			);
			if(count($r)) {
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `id` IN ( %s ) AND `blocked` = 0 AND `pending` = 0 ",
		dbesc($recip_str)
	);
		...
		foreach($r as $contact) {
			if((! $mail)  &&  (! $fsuggest)  &&  (! $followup)  &&  (! $contact['self'])) {
				if(($contact['network'] === NETWORK_DIASPORA)  &&  ($public_message))
					continue;
				q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "deliverq") . "` ( `cmd`, `item`, `contact` ) values (%s, %d, %d )",
					dbesc($cmd),
					intval($item_id),
					intval($contact['id'])
				);
		...
		if(! get_config('system','dfrn_only')) {
			foreach($url_recipients as $url) {
				if($url) {
					...
					// TODO: redeliver/queue these items on failure, though there is no `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` record
				}
			}
		}
	}
	...
	if($public_message) {
		...
		$r1 = q("SELECT DISTINCT( `batch` ), `id`, `name`, `network` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `network` = %s AND `uid` = %d AND `rel` != %d group by `batch` ORDER BY rand() ", dbesc(NETWORK_DIASPORA), intval($owner['uid']), intval(CONTACT_IS_SHARING));
			
		$r2 = q("SELECT `id`, `name`, `network` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` 
