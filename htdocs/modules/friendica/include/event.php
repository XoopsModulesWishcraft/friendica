...
function parse_event($h) {
	...
	require_once($GLOBALS['xoops']->path("/modules/friendica/include/html2bbcode"));
...
function event_store($arr) {
	...
	if($arr['cid'])
		$c = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `id` = %d AND `uid` = %d LIMIT 1",
			intval($arr['cid']),
			intval($arr['uid'])
		);
	else
		$c = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `self` = 1 AND `uid` = %d LIMIT 1",
			intval($arr['uid'])
		);
	...
	if($arr['id']) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "` WHERE `id` = %d AND `uid` = %d LIMIT 1",
			intval($arr['id']),
			intval($arr['uid'])
		);
		if((! count($r)) || ($r[0]['edited'] === $arr['edited'])) {
			...
			$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `event-id` = %d AND `uid` = %d LIMIT 1",
				intval($arr['id']),
				intval($arr['uid'])
			);
		...
		if(count($r)) {
			...
			q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `body` = %s, `object` = %s, `allow_cid` = %s, `allow_gid` = %s, `deny_cid` = %s, `deny_gid` = %s, `edited` = %s, `private` = %d WHERE `id` = %d AND `uid` = %d LIMIT 1",
		...
		$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "` ( `uid`, `cid`, `uri`, `created`, `edited`, `start`, `finish`, `desc`, `location`, `type`,
		...
		if($item_id) {
			q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `plink` = %s, `event-id` = %d  WHERE `uid` = %d AND `id` = %d LIMIT 1",
				dbesc($plink),
				intval($event['id']),
				intval($arr['uid']),
				intval($item_id)
