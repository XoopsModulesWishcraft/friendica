...
require_once($GLOBALS['xoops']->path("/modules/friendica/library/HTML5/Parser.php"));
...
/**
 *
 * PROBE_DIASPORA has a bias towards returning Diasporainformation
...
function probe_url($url, $mode = PROBE_NORMAL) {
	...
	if(! $twitter) {
		...
		if(count($links)) {
			...
			// to a `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` on incoming messages to prevent spam, AND we won't know which one
	...
	// No human could be associated with this link, use the URL as the `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` name
