...
require_once($GLOBALS['xoops']->path("/modules/friendica/include/template_processor.php"));
...
function photo_new_resource() {
	...
	do {
		...
		$r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `resource-id` = %s LIMIT 1",
			dbesc($resource)
		);
		if(count($r))
			$found = true;
...
function contact_block() {
	...
	if((! is_array($a->profile)) || ($a->profile['hide-friends']))
		return $o;
	$r = q("SELECT COUNT(*) AS `total` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `uid` = %d AND `self` = 0 AND `blocked` = 0 AND `pending` = 0 AND `hidden` = 0",
	...
	if(! $total) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `uid` = %d AND `self` = 0 AND `blocked` = 0 AND `pending` = 0 AND `hidden` = 0 ORDER BY RAND() LIMIT %d",
				intval($a->profile['uid']),
				intval($shown)
		);
		if(count($r)) {
...
function lang_selector() {
	...
	$langs = glob($GLOBALS['xoops']->path("/modules/friendica/langauge/*/strings.php"));
	if(is_array($langs)  &&  count($langs)) {
		$langs[] = '';
		if(! in_array($GLOBALS['xoops']->path("/modules/friendica/langauge/english/strings.php"),$langs))
			$langs[] = $GLOBALS['xoops']->path("/modules/friendica/langauge/english/");
		asort($langs);
		foreach($langs as $l) {
			if($l == '') {
