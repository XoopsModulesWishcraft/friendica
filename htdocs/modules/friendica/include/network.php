...
function add_fcontact($arr,$update = false) {
	...
	if($update) {
		$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fcontact") . "` SET
			`name` = %s,
			`photo` = %s,
			`request` = %s,
			`nick` = %s,
		...
		$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fcontact") . "` ( `url`, `name`, `photo`, `request`, `nick`, `addr`,
