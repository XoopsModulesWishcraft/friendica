...
function update_contact_birthdays() {
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `bd` != '' AND `bd` != '0000-00-00' AND SUBSTRING( `bd`,1,4) != `bdyear` ");
	if(count($r)) {
		foreach($r as $rr) {
			...
			$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "` ( `uid`, `cid`, `created`, `edited`, `start`, `finish`, `desc`, `type`, `adjust` )
			...
			q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `bdyear` = %s, `bd` = %s WHERE `uid` = %d AND `id` = %d LIMIT 1",
