...
require_once($GLOBALS['xoops']->path("/modules/friendica/include/config.php"));
require_once($GLOBALS['xoops']->path("/modules/friendica/include/network.php"));
require_once($GLOBALS['xoops']->path("/modules/friendica/include/plugin.php"));
require_once($GLOBALS['xoops']->path("/modules/friendica/include/text.php"));
require_once($GLOBALS['xoops']->path("/modules/friendica/include/pgettext.php"));
...
class App {
	...
	function __construct() {
		...
		/**
		 * See if there is any page numberinformation, AND initialise 
		 * pagination
		 */
...
function system_unavailable() {
	include_once($GLOBALS['xoops']->path("/modules/friendica/include/system_unavailable.php"));
	system_down();
	killme();
}}
...
function check_config(&$a) {
	...
	if($build != DB_UPDATE_VERSION) {
		...
		if(($stored < $current)  &&  file_exists('update.php')) {
			...
			require_once($GLOBALS['xoops']->path("/modules/friendica/include/update.php"));
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "addon") . "` WHERE `installed` = 1");
	if(count($r))
		$installed = $r;
	else
		$installed = array();
...
function get_guid($size=16) {
	...
	do {
		...
		$r = q("SELECT id FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "guid"). "` WHERE guid = %s LIMIT 1", dbesc($s));
		if(! count($r))
			$exists = false;
	} while($exists);
	q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "guid") . "  ( guid ) values ( %s ) ", dbesc($s));
	return $s;
}
...
// Returns `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` id of authenticated site visitor or false
 ...
 * one that has been configured for his/her viewing in the `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` manager.
 * Passing a non-zero profile ID can also allow a preview of a selected profile
 * by the owner.
 *
 * Profileinformation is placed in the App structure for later retrieval.
...
function profile_load(&$a, $nickname, $profile = 0) {
	if(remote_user()) {
		$r = q("SELECT `profile-id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `id` = %d LIMIT 1",
	...
	if($profile) {
		...
		$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`uid` AS `profile_uid`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.* , `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`avatar-date` AS picdate, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.* FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile"). "`
			LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`uid` LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid`
			WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`nickname` = %s AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`id` = %d AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`self` = 1 LIMIT 1",
			dbesc($nickname),
			intval($profile_int)
		);
	}
	if((! $r)  &&  (!  count($r))) {	
		$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`uid` AS `profile_uid`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.* , `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`avatar-date` AS picdate, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.* FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile"). "`
			LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`uid` LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid`
			WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`nickname` = %s AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`.`is-default` = 1 AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`self` = 1 LIMIT 1",
			dbesc($nickname)
		);
	}
	...
	if(! $r[0]['is-default']) {
		$x = q("SELECT `pub_keywords` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "` WHERE uid = %d AND `is-default` = 1 LIMIT 1",
			intval($profile_uid)
		);
		if($x  &&  count($x))
...
function profile_sidebar($profile, $block = 0) {
	...
	if ($profile['uid'] == local_user()) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "` WHERE `uid` = %d",
				local_user());
...
function get_birthdays() {
	...
	$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`.`id` AS `eid`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.* FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`
		LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`id` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`.`cid`
		WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`.`uid` = %d AND `type` = 'birthday' AND `start` < %s AND `finish` > %s 
		ORDER BY `start` ASC ",
		intval(local_user()),
...
function get_events() {
	...
	$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`.* FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`
		WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "event") . "`.`uid` = %d AND `type` != 'birthday' AND `start` < %s AND `start` > %s
		ORDER BY `start` ASC ",
		intval(local_user()),
		dbesc(datetime_convert('UTC','UTC','now + 6 days')),
...
function feed_birthday($uid,$tz) {
	...
	$p = q("SELECT `dob` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "` WHERE `is-default` = 1 AND `uid` = %d LIMIT 1",
		intval($uid)
	);
...
function load_contact_links($uid) {
	...
	$r = q("SELECT `id`, `network`, `url`, `thumb` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `uid` = %d AND `self` = 0 AND `blocked` = 0 ",
			intval($uid)
	);
	if(count($r)) {
		foreach($r as $rr){
