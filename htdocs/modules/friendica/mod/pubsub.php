...
function pubsub_init(&$a) {
	...
	if($_SERVER['REQUEST_METHOD'] === 'GET') {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `nickname` = %s AND `account_expired` = 0 LIMIT 1",
			dbesc($nick)
		);
		if(! count($r))
			hub_return(false, '');
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `poll` = %s AND `id` = %d AND `uid` = %d 
		...
		$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `subhub` = %d WHERE `id` = %d LIMIT 1",
			intval($subscribe),
			intval($contact['id'])
		);
...
//	}
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `subhub` = 1 AND `id` = %d AND `uid` = %d 
