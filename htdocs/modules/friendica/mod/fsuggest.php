...
function fsuggest_post(&$a) {
	...
	if($new_contact) {
		...
		if(count($r)) {
			...
			$x = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fsuggest") . "` ( `uid`, `cid`, `name`, `url`, `request`, `photo`, `note`, `created` )
			...
			$r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fsuggest") . "` WHERE `note` = %s AND `uid` = %d LIMIT 1",
				dbesc($hash),
				intval(local_user())
			);
			if(count($r)) {
				$fsuggest_id = $r[0]['id'];
				q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "fsuggest") . " SET `note` = %s WHERE `id` = %d AND `uid` = %d LIMIT 1",
					dbesc($note),
					intval($fsuggest_id),
					intval(local_user())
				);
