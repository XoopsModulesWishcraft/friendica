...
function manage_post(&$a) {
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "manager") . "` WHERE uid = %d",
		intval($uid)
	);
	...
	if($limited_id) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `uid` = %d AND `email` = %s AND `password` = %s LIMIT 1",
			intval($identity),
