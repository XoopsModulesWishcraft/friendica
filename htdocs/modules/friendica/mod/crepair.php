...
function crepair_post(&$a) {
	...
	$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `name` = %s, `nick` = %s, `url` = %s, `request` = %s, `confirm` = %s, `notify` = %s, `poll` = %s, `attag` = %s 
	...
	if($photo) {
		...
		$x = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `photo` = %s,
			`thumb` = %s,
			`micro` = %s,
			`name-date` = %s,
			`uri-date` = %s,
...
function crepair_content(&$a) {
	...
	$msg1 = t('Repair `' . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . '` Settings');

	$msg2 = t('<strong>WARNING: This is highly advanced</strong> AND if you enter incorrectinformation your communications with this contact may stop working.');
