...
function photos_init(&$a) {
	...
	if($a->argc > 1) {
		$nick = $a->argv[1];
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `nickname` = %s AND `blocked` = 0 LIMIT 1",
			dbesc($nick)
		);
		...
		$albums = q("SELECT distinct( `album` ) AS `album` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d $sql_extra ",
		...
		if(count($albums)) {
			...
			foreach($albums as $album) {
				...
				// don't show `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` photos. We once translated this name, but then you could still access it under
...
function photos_post(&$a) {
	...
	if((local_user())  &&  (local_user() == $page_owner_uid))
		...
		if($community_page  &&  remote_user()) {
			$r = q("SELECT `uid` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `blocked` = 0 AND `pending` = 0 AND `id` = %d AND `uid` = %d LIMIT 1",
				intval(remote_user()),
	...
	$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`nickname` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid`
		WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid` = %d AND `self` = 1 LIMIT 1",
		intval($page_owner_uid)
	);

	if(! count($r)) {
		notice( t('Contactinformation unavailable') . EOL);
	...
	if(($a->argc > 3)  &&  ($a->argv[2] === 'album')) {
		...
		$r = q("SELECT count(*) FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `album` = %s AND `uid` = %d",
			dbesc($album),
			intval($page_owner_uid)
		);
		if(! count($r)) {
		...
		if($newalbum != $album) {
			q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` SET `album` = %s WHERE `album` = %s AND `uid` = %d",
				dbesc($newalbum),
				dbesc($album),
				intval($page_owner_uid)
			);
		...
		if($_POST['dropalbum'] == t('Delete Album')) {
			...
			if($visitor) {
				$r = q("SELECT distinct( `resource-id` ) as `rid` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `contact-id` = %d AND `uid` = %d AND `album` = %s",
					intval($visitor),
					intval($page_owner_uid),
					dbesc($album)
				);
				...
				$r = q("SELECT distinct( `resource-id` ) as `rid` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `album` = %s",
					intval(local_user()),
					dbesc($album)
				);
			}
			...
			q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `resource-id` IN ( $str_res ) AND `uid` = %d",
				intval($page_owner_uid)
			);

			// find AND delete the corresponding item with all the comments AND likes/dislikes

			$r = q("SELECT `parent-uri` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `resource-id` IN ( $str_res ) AND `uid` = %d",
				intval($page_owner_uid)
			);
			if(count($r)) {
				foreach($r as $rr) {
					q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `deleted` = 1, `changed` = %s WHERE `parent-uri` = %s AND `uid` = %d",
	...
	if(($a->argc > 2)  &&  (x($_POST,'delete'))  &&  ($_POST['delete'] == t('Delete Photo'))) {
		...
		if($visitor) {
			$r = q("SELECT `id`, `resource-id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `contact-id` = %d AND `uid` = %d AND `resource-id` = %s LIMIT 1",
				intval($visitor),
				intval($page_owner_uid),
				dbesc($a->argv[2])
			);
			...
			$r = q("SELECT `id`, `resource-id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `resource-id` = %s LIMIT 1",
				intval(local_user()),
				dbesc($a->argv[2])
			);
		}
		if(count($r)) {
			q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `resource-id` = %s",
				intval($page_owner_uid),
				dbesc($r[0]['resource-id'])
			);
			$i = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `resource-id` = %s AND `uid` = %d LIMIT 1",
				dbesc($r[0]['resource-id']),
				intval($page_owner_uid)
			);
			if(count($i)) {
				q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `deleted` = 1, `edited` = %s, `changed` = %s WHERE `parent-uri` = %s AND `uid` = %d",
					dbesc(datetime_convert()),
					dbesc(datetime_convert()),
					dbesc($i[0]['uri']),
					intval($page_owner_uid)
	...
	if(($a->argc > 2)  &&  ((x($_POST,'desc') !== false) || (x($_POST,'newtag') !== false)) || (x($_POST,'albname') !== false)) {
		...
		$p = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `resource-id` = %s AND `uid` = %d ORDER BY `scale` DESC",
			dbesc($resource_id),
			intval($page_owner_uid)
		);
		if(count($p)) {
			$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` SET `desc` = %s, `album` = %s, `allow_cid` = %s, `allow_gid` = %s, `deny_cid` = %s, `deny_gid` = %s WHERE `resource-id` = %s AND `uid` = %d",
				dbesc($desc),
				dbesc($albname),
		...
		if(strlen($rawtags)) {
					...
					if(strpos($tag,'@') === 0) {
						...
						if((strpos($name,'@')) || (strpos($name,'http://'))) {
							...
							elseif(strstr($name,'_') || strstr($name,' ')) {
								...
								$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `name` = %s AND `uid` = %d LIMIT 1",
									dbesc($newname),
									intval($page_owner_uid)
								);
							}
							else {
								$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `attag` = %s OR `nick` = %s AND `uid` = %d ORDER BY `attag` DESC LIMIT 1",
									dbesc($name),
									dbesc($name),
									intval($page_owner_uid)
								);
			...
			$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `tag` = %s, `inform` = %s, `edited` = %s, `changed` = %s WHERE `id` = %d AND `uid` = %d LIMIT 1",
				dbesc($newtag),
				dbesc($newinform),
				dbesc(datetime_convert()),
				dbesc(datetime_convert()),
			...
			if(count($taginfo)) {
				foreach($taginfo as $tagged) {
					...
					$item_id = item_store($arr);
					if($item_id) {
						q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `plink` = %s WHERE `uid` = %d AND `id` = %d LIMIT 1",
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `album` = %s AND `uid` = %d AND `created` > UTC_TIMESTAMP() - INTERVAL 3 HOUR ",
		dbesc($album),
		intval($page_owner_uid)
	);
...
function photos_content(&$a) {
	...
	if(remote_user()  &&  (! $visitor)) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `blocked` = 0 AND `pending` = 0 AND `id` = %d AND `uid` = %d LIMIT 1",
			intval(remote_user()),
			intval($owner_uid)
		);
		if(count($r)) {
	...
	if($datatype === 'album') {
		...
		$r = q("SELECT `resource-id`, max( `scale` ) AS `scale` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `album` = %s 
		...
		$r = q("SELECT `resource-id`, `id`, `filename`, max( `scale` ) AS `scale`, `desc` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `album` = %s 
	...
	if($datatype === 'image') {
		...
		$ph = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `resource-id` = %s 
		...
		if(! count($ph)) {
			$ph = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `resource-id` = %s 
				LIMIT 1",
				intval($owner_uid),
				dbesc($datum)
			);
		...
		$prvnxt = q("SELECT `resource-id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `album` = %s AND `uid` = %d AND `scale` = 0 
		...
		$linked_items = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `resource-id` = %s $sql_extra LIMIT 1",
			dbesc($datum)
		);
		if(count($linked_items)) {
			...
			$r = q("SELECT COUNT(*) AS `total`
				...
				WHERE `parent-uri` = %s AND `uri` != %s AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`deleted` = 0 AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`moderated` = 0
			...
			$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`id` AS `item_id`, 
				`" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`name`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` , `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`url`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`network`, 
				`" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`rel`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`thumb`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`self`, 
			...
			if((local_user())  &&  (local_user() == $link_item['uid'])) {
				q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `unseen` = 0 WHERE `parent` = %d AND `uid` = %d",
	...
	$r = q("SELECT `resource-id`, max( `scale` ) AS `scale` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `uid` = %d AND `album` != %s AND `album` != %s 
	...
	$r = q("SELECT `resource-id`, `id`, `filename`, `album`, max( `scale` ) AS `scale` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo"). "`
