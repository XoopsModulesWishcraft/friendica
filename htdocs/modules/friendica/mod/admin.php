<?php
  ...
  * Friendica admin
  */
require_once($GLOBALS['xoops']->path("/modules/friendica/include/remoteupdate.php"));
			...
			case 'plugins':
				if ($a->argc > 2  &&  
					is_file("addon/".$a->argv[2]."/".$a->argv[2].".php")){
						@include_once($GLOBALS['xoops']->path("/modules/friendica/addon/".$a->argv[2]."/".$a->argv[2].".php"));
...
function admin_content(&$a) {
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "addon") . "` WHERE `plugin_admin`=1");
...
function admin_page_summary(&$a) {
	$r = q("SELECT `page-flags`, COUNT(`uid`) as `count` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` GROUP BY `page-flags`");
	$accounts = Array(
	...
	$r = q("SELECT COUNT(`id`) as `count` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "register") . "`");
	$pending = $r[0]['count'];
...
function admin_page_site_post(&$a){
	...
	if ($banner==""){
		...
		q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "config") . "` WHERE `cat` = %s AND `k` = %s LIMIT 1",
			dbesc("system"),
			dbesc("banner")
		);
	} else {
...
function admin_page_site(&$a) {
	...
	/* installed langs */
	$lang_choices = array();
	$langs = glob($GLOBALS['xoops']->path("/modules/friendica/language/*/strings.php"));
	
	if(is_array($langs)  &&  count($langs)) {
		if(! in_array($GLOBALS['xoops']->path("/modules/friendica/language/english/strings.php"),$langs))
			$langs[] = $GLOBALS['xoops']->path("/modules/friendica/language/english/");
		asort($langs);
		foreach($langs as $l) {
			$t = explode("/",$l);
	...
	/* installed themes */
	$theme_choices = array();
	$files = glob($GLOBALS['xoops']->path("/modules/friendica/theme/*"));
	if($files) {
...
function admin_page_users_post(&$a){
	...
	if (x($_POST,'page_users_block')){
		foreach($users as $uid){
			q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` SET `blocked` = 1 - `blocked` WHERE `uid`=%s",
				intval( $uid )
			);
		}
	...
	if (x($_POST,'page_users_approve')){
		require_once($GLOBALS['xoops']->path("/modules/friendica/mod/regmod.php"));
...
function admin_page_users(&$a){
	if ($a->argc>2) {
		$uid = $a->argv[3];
		$user = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `uid`=%d", intval($uid));
		if (count($user)==0){
		...
		switch($a->argv[2]){
			...
			case "block":{
				q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` SET `blocked`=%d WHERE `uid`=%s",
	...
	/* get pending */
	$pending = q("SELECT `register`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`name`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`email`
				 FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "register"). "`
				 LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` ON `register`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid`
				 LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ON `register`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid`;");
	
	
	/* get users */

	$total = q("SELECT count(*) as total FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE 1");
	if(count($total)) {
	...
	$users = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`. * , `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`name` , `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`url` , `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`micro`, `lastitem`.`lastitem_date`
				FROM
					(SELECT MAX( `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`changed` ) as `lastitem_date`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`uid`
					FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` 
					WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`type` = 'wall'
					GROUP BY `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "`.`uid` ) AS `lastitem`
						 RIGHT OUTER JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid` = `lastitem`.`uid`,
					   `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` 
				WHERE
					   `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid`
						AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`verified` = 1
					AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`self` = 1
				ORDER BY `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`name` LIMIT %d, %d
				",
...
function admin_page_plugins(&$a){
	...
	if ($a->argc == 3){
		...
		require_once($GLOBALS['xoops']->path("/modules/friendica/library/markdown.php"));
		...
		if (is_array($a->plugins_admin)  &&  in_array($plugin, $a->plugins_admin)){
			@require_once($GLOBALS['xoops']->path("/modules/friendica/include/addon/$plugin/$plugin.php"));
