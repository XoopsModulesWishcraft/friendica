...
//	logger('postinput ' . file_get_contents('php://input'));
	...
	if($parent || $parent_uri) {
		...
		if($r !== false  &&  count($r)) {
			...
			if($r[0]['id'] != $r[0]['parent']) {
				$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `id` = `parent` AND `parent` = %d LIMIT 1",
					intval($parid)
				);
			}
		}
	...
	if($post_id) {
		$i = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `uid` = %d AND `id` = %d LIMIT 1",
			intval($profile_uid),
			intval($post_id)
		);
		if(! count($i))
	...
	if($orig_post) {
		...
		if($api_source  &&  $profile_uid  &&  $profile_uid == local_user()  &&  (! $private)) {
			...
			if(! $mail_disabled) {
				$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mailacct") . "` WHERE `uid` = %d AND `server` != '' LIMIT 1",
					intval(local_user())
				);
	...
	// get `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`info for poster
	...
	// get `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`info for owner
	...
	if((! $preview)  &&  preg_match_all("/\[img\](.*?)\[\/img\]/",$body,$match)) {
		...
		if(count($images)) {
			foreach($images as $image) {
				...
				$r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` WHERE `allow_cid` = %s AND `allow_gid` = '' AND `deny_cid` = '' AND `deny_gid` = ''
				...
				$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` SET `allow_cid` = %s, `allow_gid` = %s, `deny_cid` = %s, `deny_gid` = %s
	...
	if((! $preview)  &&  preg_match_all("/\[attachment\](.*?)\[\/attachment\]/",$body,$match)) {
		...
		if(count($attaches)) {
			foreach($attaches as $attach) {
				$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "attach") . "` WHERE `uid` = %d AND `id` = %d LIMIT 1",
					intval($profile_uid),
					intval($attach)
				);				
				if(count($r)) {
					$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "attach") . " SET `allow_cid` = %s, `allow_gid` = %s, `deny_cid` = %s, `deny_gid` = %s
	...
	if(preg_match_all('/(\[attachment\]([0-9]+)\[\/attachment\])/',$body,$match)) {
		foreach($match[2] as $mtch) {
			$r = q("SELECT `id`, `filename`, `filesize`, `filetype` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "attach") . "` WHERE `uid` = %d AND `id` = %d LIMIT 1",
				intval($profile_uid),
				intval($mtch)
			);
			if(count($r)) {
...
//	$datarray['prvnets']       = $user['prvnets'];
	...
	if($orig_post) {
		$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `title` = %s, `body` = %s, `edited` = %s WHERE `id` = %d AND `uid` = %d LIMIT 1",
			dbesc($title),
			dbesc($body),
	...
	$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` ( `guid`, `uid`, `type`, `wall`, `gravity`, `contact-id`, `owner-name`, `owner-link`, `owner-avatar`, 
		`author-name`, `author-link`, `author-avatar`, `created`, `edited`, `commented`, `received`, `changed`, `uri`, `thr-parent`, `title`, `body`, `app`, `location`, `coord`, 
		`tag`, `inform`, `verb`, `postopts`, `allow_cid`, `allow_gid`, `deny_cid`, `deny_gid`, `private`, `pubmail`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "attach") . " , `bookmark`, `origin`, `moderated` ) 
	...
	$r = q("SELECT `id` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` WHERE `uri` = %s LIMIT 1",
		dbesc($datarray['uri']));
	if(count($r)) {
		$post_id = $r[0]['id'];
		...
		if($parent) {
			...
			$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `last-child` = 0, `changed` = %s WHERE `parent` = %d ",
				dbesc(datetime_convert()),
				intval($parent)
			);
			...
			$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `allow_cid` = %s, `allow_gid` = %s, `deny_cid` = %s, `deny_gid` = %s, `private` = %d
				WHERE `id` = %d LIMIT 1",
		...
		$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `parent` = %d, `parent-uri` = %s, `plink` = %s, `changed` = %s, `last-child` = 1, `visible` = 1
			WHERE `id` = %d LIMIT 1",
			intval($parent),
		...
		if(! $parent_item['visible']) {
			$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "item") . "` SET `visible` = 1 WHERE `id` = %d LIMIT 1",
