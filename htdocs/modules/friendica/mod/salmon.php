...
function salmon_post(&$a) {
	...
	*/
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `network` = 'stat' AND ( `url` = %s OR `alias` = %s) 
		AND `uid` = %d LIMIT 1",
		dbesc($author_link),
		dbesc($author_link),
		intval($importer['uid'])
	 ...
	 * this person (and nothing else) if there is no existing `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` record.
	 *
	 */
