...
function register_post(&$a) {
	...
	if($max_dailes) {
		$r = q("SELECT count(*) as total FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE register_date > UTC_TIMESTAMP - INTERVAL 1 day");
	...
	if((! x($username)) || (! x($email)) || (! x($nickname))) {
		...
		notice( t('Please enter the requiredinformation.') . EOL );
		return;
	}
	...
	if((x($a->config,'admin_email'))  &&  (strcasecmp($email,$a->config['admin_email']) == 0)  &&  strlen($openid_url)) {
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `email` = %s LIMIT 1",
			dbesc($email)
		);
		if(count($r))
	...
	$r = q("SELECT `uid` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`
	...
	$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ( `guid`, `username`, `password`, `email`, `openid`, `nickname`,
	...
	if((count($r) > 1)  &&  $newuid) {
		$err .= t('Nickname is already registered. Please choose another.') . EOL;
		q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `uid` = %d LIMIT 1",
			intval($newuid)
		);
		notice ($err);
		return;
	...
	if(x($newuid) !== false) {
		$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "` ( `uid`, `profile-name`, `is-default`, `name`, `photo`, `thumb`, `publish`, `net-publish` ) 
		...
		if($r === false) {
			...
			$r = q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `uid` = %d",
				intval($newuid));
			return;
		}
		$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact")  . "` ( `uid`, `created`, `self`, `name`, `nick`, `photo`, `thumb`, `micro`, `blocked`, `pending`, `url`, `nurl`,
	...
	if($use_gravatar || strlen($photo)) {
		...
		if($img->is_valid()) {
			...
			if(! $photo_failure) {
				q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "photo") . "` SET `profile` = 1 WHERE `resource-id` = %s ",
					dbesc($hash)
				);
			}
		}
	...
	elseif($a->config['register_policy'] == REGISTER_APPROVE) {
		...
		$hash = random_string();
		$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "register") . "` ( `hash`, `created`, `uid`, `password`, `language` ) VALUES ( %s, %s, %d, %s, %s ) ",
			dbesc($hash),
			dbesc(datetime_convert()),
			intval($newuid),
			dbesc($new_password),
		...
		$r = q("SELECT `language` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `email` = %s LIMIT 1",
