...
function settings_post(&$a) {
	...
	if(($a->argc > 1)  &&  ($a->argv[1] === 'oauth')  &&  x($_POST,'remove')){
		$key = $_POST['remove'];
		q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "tokens") . "` WHERE id=%s AND uid=%d",
			dbesc($key),
			local_user());
	...
	if(($a->argc > 2)  &&  ($a->argv[1] === 'oauth')   &&  ($a->argv[2] === 'edit'||($a->argv[2] === 'add'))  &&  x($_POST,'submit')) {
		...
		if ($name=="" || $key=="" || $secret==""){
			...
			if ($_POST['submit']==t("Update")){
				$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "  SET
							client_id=%s,
							pw=%s,
							name=%s,
							redirect_uri=%s,
				...
				$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "`
	...
	if(($a->argc > 1)  &&  ($a->argv[1] == 'connectors')) {
		...
		if(x($_POST['imap-submit'])) {
			...
			if(! $mail_disabled) {
				$failed = false;
				$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mailacct") . "` WHERE `uid` = %d LIMIT 1",
					intval(local_user())
				);
				if(! count($r)) {
					q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mailacct") . "` ( `uid` ) VALUES (%d)",
						intval(local_user())
					);
				}
				if(strlen($mail_pass)) {
					...
					q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mailacct") . "` SET `pass` = %s WHERE `uid` = %d LIMIT 1",
						dbesc(bin2hex($pass)),
						intval(local_user())
					);
				}
				$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "mailacct") . "` SET `server` = %s, `port` = %d, `ssltype` = %s,  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`  = %s,
	...
	if((x($_POST,'npassword')) || (x($_POST,'confirm'))) {
		...
		if(! $err) {
			...
			$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` SET `password` = %s WHERE `uid` = %d LIMIT 1",
				dbesc($password),
				intval(local_user())
			);
			if($r)
	...
	$r = q("UPDATE  `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` SET `username` = %s, `email` = %s, `openid` = %s, `timezone` = %s,  `allow_cid` = %s, `allow_gid` = %s, `deny_cid` = %s, `deny_gid` = %s, `notify-flags` = %d, `page-flags` = %d, `default-location` = %s, `allow_location` = %d, `theme` = %s, `maxreq` = %d, `expire` = %d, `openidserver` = %s, `blockwall` = %d, `hidewall` = %d, `blocktags` = %d  WHERE `uid` = %d LIMIT 1",
			dbesc($username),
			dbesc($email),
			dbesc($openid),
			dbesc($timezone),
	...
	$r = q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "`
		SET `publish` = %d, 
		`net-publish` = %d,
		`hide-friends` = %d
	...
	if($name_change) {
		q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` SET `name` = %s, `name-date` = %s WHERE `uid` = %d AND `self` = 1 LIMIT 1",
			dbesc($username),
	...
	require_once($GLOBALS['xoops']->path("/modules/friendica/include/profile_update.php"));
	profile_change();
...
function settings_content(&$a) {
	...
	if(($a->argc > 1)  &&  ($a->argv[1] === 'oauth')) {
		...
		if(($a->argc > 3)  &&  ($a->argv[2] === 'edit')) {
			$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "` WHERE client_id=%s AND uid=%d",
					dbesc($a->argv[3]),
					local_user());
		...
		if(($a->argc > 3)  &&  ($a->argv[2] === 'delete')) {
			$r = q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "` WHERE client_id=%s AND uid=%d",
					dbesc($a->argv[3]),
					local_user());
			goaway($a->get_baseurl()."/settings/oauth/");
			return;			
		...
		$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "tokens") . "`.id as oauth_token, ( `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "`.uid=%d) AS my 
				FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "`
				LEFT JOIN tokens ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "`.client_id=`" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "tokens") . "`.client_id
				WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "clients") . "`.uid IN (%d,0)",
				local_user(),
				local_user());
	...
	if(($a->argc > 1)  &&  ($a->argv[1] === 'addon')) {
		...
		$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "hook") . "` WHERE `hook` = 'plugin_settings' ");
		if(! count($r))
	...
	$p = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile") . "` WHERE `is-default` = 1 AND `uid` = %d LIMIT 1",
		intval(local_user())
	);
	if(count($p))
		$profile = $p[0];
	...
	$o .= replace_macros($stpl,array(
		...
		'$notify1'	=> array('notify1', t('You receive anintroduction'), ($notify & NOTIFY_intro), NOTIFY_intro, ''),
		'$notify2'	=> array('notify2', t('Yourintroductions are confirmed'), ($notify & NOTIFY_CONFIRM), NOTIFY_CONFIRM, ''),
