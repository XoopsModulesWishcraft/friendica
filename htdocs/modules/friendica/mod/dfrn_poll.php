...
require_once($GLOBALS['xoops']->path("/modules/friendica/include/auth.php"));
...
function dfrn_poll_init(&$a) {
	...
	if(($dfrn_id === '')  &&  (! x($_POST,'dfrn_id'))  &&  ($a->argc > 1)) {
		...
		$r = q("SELECT `hidewall` FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` WHERE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`nickname` = %s LIMIT 1",
			dbesc($a->argv[1])
		);
	...
	if(($type === 'profile')  &&  (! strlen($sec))) {
		...
		$r = q("SELECT `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.*, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`username`, `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`nickname`
			FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` LEFT JOIN `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "` ON `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "`.`uid` = `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`uid`
			...
			AND `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "user") . "`.`nickname` = %s $sql_extra LIMIT 1",
			dbesc($a->argv[1])
		);
		...
		if(count($r)) {
			...
			if(strlen($s)) {
				...
				if((int) $xml->status == 1) {
					...
					$expire = time() + 86400;
					q("UPDATE `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "session") . "` SET `expire` = %s WHERE `sid` = %s LIMIT 1",
						dbesc($expire),
						dbesc($session_id)
					); 
				}
	...
	if($type === 'profile-check'  &&  $dfrn_version < 2.2 ) {
		...
		if((strlen($challenge))  &&  (strlen($sec))) {
			...
			q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile_check") . "` WHERE `expire` < " . intval(time()));
			$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile_check") . "` WHERE `sec` = %s ORDER BY `expire` DESC LIMIT 1",
				dbesc($sec)
			);
			if(! count($r)) {
			...
			q("DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile_check") . "` WHERE `expire` < " . intval(time()));
			$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "profile_check") . "` WHERE `dfrn_id` = %s ORDER BY `expire` DESC",
				dbesc($dfrn_id));
			if(count($r)) {
				xml_status(1);
				return; // NOTREACHED
...
function dfrn_poll_post(&$a) {
	...
	$r = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "contact") . "` WHERE `blocked` = 0 AND `pending` = 0 $sql_extra LIMIT 1");
...
function dfrn_poll_content(&$a) {
	...
	if($dfrn_id != '') {
		...
		if($type !== 'profile') {
			$r = q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "challenge") . "` ( `challenge`, `dfrn-id`, `expire`, `type`, `last_update` )
				VALUES( %s, %s, %s, %s, %s ) ",
				dbesc($hash),
				dbesc($dfrn_id),
				intval(time() + 60 ),
