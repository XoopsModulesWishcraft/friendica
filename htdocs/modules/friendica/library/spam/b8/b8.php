...
class b8
{
	...
	public function classify($uid,$text)
	{
		...
		# Get the mostinteresting tokens (use all if we have less than the given number)
	...
	private function _get_probability($word, $texts_ham, $texts_spam)
	{
		...
		if(isset($this->_token_data['degenerates'][$word]) === TRUE) {
			...
			# for completely unknown `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "tokens") . "`. This strips down to the
