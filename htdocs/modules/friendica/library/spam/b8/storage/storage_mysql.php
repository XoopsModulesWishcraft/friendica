...
class b8_storage_mysql extends b8_storage_base
{
	...
	public function connect()
	{
		...
		if($GLOBALS['xoopsDB']->queryF('DESCRIBE ' . $this->config['table_name'], $this->_connection) === FALSE) {
			$this->connected = FALSE;
	...
	protected function _get_query($tokens)
	{
		...
		$result = $GLOBALS['xoopsDB']->queryF('
			SELECT token, count
			FROM `' . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '`
			WHERE ' . $where . ';
		', $this->_connection);

		$data = array();

		while ($row = $GLOBALS['xoopsDB']->fetchArray($result, MYSQL_ASSOC))
	...
	protected function _commit()
	{
		...
		if(count($this->_puts) > 0) {
			...
			$result = $GLOBALS['xoopsDB']->queryF('
				INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '`(token, count)
