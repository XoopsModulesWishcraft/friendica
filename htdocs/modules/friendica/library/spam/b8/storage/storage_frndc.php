...
class b8_storage_frndc extends b8_storage_base
{
	...
	protected function _get_query($tokens, $uid)
	{
		...
		$result = q('
			SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "spam") . "` WHERE ' . $where . ' AND uid = ' . $uid );
		...
		if(count($to_create)) {
			...
			q("INSERT INTO `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "spam") . "` " . $sql);
		}
	...
	protected function _commit($uid)
	{
		...
		if(count($this->_deletes) > 0) {
			...
			$result = q('
				DELETE FROM `' . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '`
		...
		if(count($this->_puts) > 0) {
			...
			$result = q('
				INSERT INTO `' . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '` (token, count, uid)
		...
		if(count($this->_updates) > 0) {
			...
			$result = q("SELECT * FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '` WHERE token = ');

			
			$result = q('
				INSERT INTO `' . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '` (token, count, uid)
				VALUES ' . implode(', ', $this->_updates) . ', ' . $uid . '
				ON DUPLICATE KEY UPDATE `' . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . $this->config['table_name']) . '`.`count` = VALUES(count);', $this->_connection);
