...
elseif(isset($_POST['action']) AND $_POST['text'] !=  "") {
	...
	switch($_POST['action']) {
		...
		case "DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "spam") . "` ":
			$b8->unlearn($text, b8::SPAM);
			echo "<p style=\"color:green\">Deleted the text FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "spam") . "` </p>\n\n";
			break;
...
<tr>
<td><input type="submit" name="action" value="DELETE FROM `" . $GLOBALS['xoopsDB']->prefix(_MI_FDC_MODULE_DB_PREFIX . "spam") . "` " /></td>
